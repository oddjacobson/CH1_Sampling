...
```{r}
df <- readRDS("../Intermediate/DF_Performance_redo2.rds") %>% 
  clean_names() %>% 
  ...
  mutate(group = factor(group),
         type = factor(ifelse(type == "R", "spread", "clumped")),
         id = gsub("R", "S", id),
...
ggplot(data = m_type_pred, aes(x = type, y = .epred_prop)) +
  ...
  labs(x = "type", y = "Performance Value") +
  theme_clean() +
...
ggplot(data = post_loc_draws, aes(x = locations_std, y = .pred_prop, color = type, fill = type)) +
  ...
  geom_hline(yintercept = 0.95, linetype = "dashed", color = "black", size = 1, alpha = 0.5) +
  geom_segment(aes(x = 1.15, xend = 1.15, y = -Inf, yend = 0.95),
               linetype = "dashed",
               color = "#370499",
               size = 1,
               alpha = 0.5) +
  geom_segment(aes(x = 0.9, xend = 0.9, y = -Inf, yend = 0.95),
               linetype = "dashed",
               color = "#feb82c",
               size = 1,
               alpha = 0.5) +
  ...
  labs(x = "Fine-Scale Volume (# locations)",
       y = "Performance Value") +
  theme_clean() +
  theme_bw(base_family = "ArcherPro Book") +
...
ggplot(data = post_weeks_draws, aes(x = weeks_std, y = .pred_prop, color = type, fill = type)) +
  ...
  geom_line(data = mean_only, stat = "smooth", # add mean line
            aes(group = type), lwd = 1.1) +
  geom_hline(yintercept = 0.95, linetype = "dashed", color = "black", size = 1, alpha = 0.5) +
  geom_segment(aes(x = 1.4, xend = 1.4, y = -Inf, yend = 0.95),
               linetype = "dashed",
               color = "#feb82c",
               size = 1,
               alpha = 0.5) +
  geom_segment(aes(x = 0.35, xend = 0.35, y = -Inf, yend = 0.95),
               linetype = "dashed",
               color = "#370499",
               size = 1,
               alpha = 0.5) +
...
ame_locs_clumped <- m_locs %>% 
  emtrends(~ locations_std*type, var = "locations_std",
           at = list(type = "clumped", locations_std = c(-1.3, -0.8, -0.3, 0.2)),
           regrid = "response") %>% 
  gather_emmeans_draws()


p1 <- ggplot(ame_locs_clumped, aes(x = .value, fill = factor(locations_std))) +
...
p2 <- ggplot(ame_locs_spread, aes(x = .value, fill = factor(locations_std))) +
  geom_vline(xintercept = 0) +
  stat_halfeye(.width = c(0.8, 0.95), point_interval = "median_hdi",
               slab_alpha = 0.75) +
...
ame_weeks_clumped <- m_weeks %>% 
  emtrends(~ weeks_std*type, var = "weeks_std",
           at = list(type = "clumped", weeks_std = c(-1.3, -0.8, -0.3, 0.2)),
           regrid = "response") %>% 
  gather_emmeans_draws()


p3 <- ggplot(ame_weeks_clumped, aes(x = .value, fill = factor(weeks_std))) +
  geom_vline(xintercept = 0) +
  stat_halfeye(.width = c(0.8, 0.95), point_interval = "median_hdi",
               slab_alpha = 0.75) +
  scale_fill_viridis_d(option = "plasma", begin = 0, end = 0.4) +
...
p4 <- ggplot(ame_weeks_spread, aes(x = .value, fill = factor(weeks_std))) +
  geom_vline(xintercept = 0) +
  stat_halfeye(.width = c(0.8, 0.95), point_interval = "median_hdi",
               slab_alpha = 0.75) +
  scale_fill_viridis_d(option = "plasma", begin = 0.6, end = 1) +
